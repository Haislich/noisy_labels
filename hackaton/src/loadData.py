...
class GraphDataset(Dataset):
    ...
    def _count_graphs(self):
        with gzip.open(self.raw, "rt", encoding="utf-8") as f:
            graphs_dicts = json.load(
                f
            )  # Load full JSON array without keeping references
...
def dictToGraphObject(graph_dict):
    edge_index = torch.tensor(graph_dict["edge_index"], dtype=torch.long)
    edge_attr = (
        torch.tensor(graph_dict["edge_attr"], dtype=torch.float)
        if graph_dict["edge_attr"]
        else None
    )
    num_nodes = graph_dict["num_nodes"]
    y = (
        torch.tensor(graph_dict["y"][0], dtype=torch.long)
